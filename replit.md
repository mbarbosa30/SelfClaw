...
## Recent Changes
- **2026-02-16**: Escrow-protected x402 commerce — platform wallet (CELO_PRIVATE_KEY) now serves as escrow facilitator for skill purchases. Buyer pays escrow, platform verifies onchain, releases to seller after delivery. Added `releaseEscrow()` and `refundEscrow()` functions. Updated agent briefing with full marketplace section (browse skills/services/agents, purchase flow, 402 payment instructions). Updated gateway action types to include browse_skills, browse_services, browse_agents. Added commerce context to `buildAgentContext` (marketplace stats, purchase history, capability list). Updated Miniclaw system prompt with marketplace endpoint documentation. Fixed all Drizzle `ne` operator compatibility issues with `sql` templates.
