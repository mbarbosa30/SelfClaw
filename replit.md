...
## Recent Changes
- **2026-02-17**: User Dashboard Health Overview — Enhanced /v1/human/:humanId/economics API to include per-agent reputation score (validated/slashed/badges), skills (count/sales/rating), commerce (provided/requested), and computed health status (setup/building/active/healthy/critical). Frontend my-agents.html now renders a health overview card per agent with status indicator, pipeline progress bar, Net P/L, reputation score, skills, commerce, and services — all visible at a glance without expanding briefings.
