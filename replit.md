...
- **OpenClaw Skill**: Added YAML frontmatter to `public/skill.md` making it a proper OpenClaw skill definition. Landing page links to `selfclaw.ai/skill.md` for agent consumption.
- **Trustless ZK Proof Sharing**: ZK proof data (proof, publicSignals, attestationId) is now stored in verifiedBots metadata on verification. New endpoint `GET /api/selfclaw/v1/agent/{identifier}/proof` returns the raw proof with step-by-step Self.xyz SDK verification instructions. Agent lookup API now includes `proof.available`, `proof.hash`, and `proof.endpoint` fields. Third parties can independently verify any agent's human backing without trusting SelfClaw.
- **Trustless Verification Documentation**: Added trustless proof verification section to Developers page with full code examples (fetch proof + verify with Self.xyz SDK), added proof endpoint to API Reference. Added Trustless Verification section to How It Works page with endpoint overview and link to developer docs. Added "Trustless" card to landing page Why SelfClaw section.
- **ERC-8004 Reputation Registry Integration**: Full integration with Celo mainnet Reputation Registry (`0x8004BAa17C55a88189AE136b182e5fdA19dE9b63`). New public API endpoints: `POST /api/selfclaw/v1/reputation/attest` submits SelfClaw verification attestation for agents with ERC-8004 tokens. `GET /api/selfclaw/v1/agent/{identifier}/reputation` returns on-chain reputation summary, all feedback entries, and attestation status. Agent lookup API now includes `reputation` field with `hasErc8004`, `endpoint`, and `registryAddress`. Added `readAllFeedback()` and `getReputationRegistryAddress()` to `lib/erc8004.ts`. Updated Developers page API reference and How It Works page with Reputation Registry documentation.
- **Hackathon-Ready Security Audit**: All write endpoints (deploy-token, transfer-token, create-wallet, request-gas, register-erc8004, create-sponsored-lp, reputation/attest, reputation/feedback) now require Ed25519 signature authentication with nonce-based replay protection. Agents sign `{"agentPublicKey":"...","timestamp":...,"nonce":"..."}` with their private key. 5-minute timestamp window + unique nonce per request.
- **Debug Endpoint Protection**: `/v1/debug-status`, `/v1/debug-callback`, `/v1/callback-test` now require `X-Admin-Secret` header matching `SELFCLAW_ADMIN_SECRET` or `SESSION_SECRET`.
- **Reputation Leaderboard**: New `GET /v1/reputation-leaderboard` endpoint fetches all agents with ERC-8004 tokens, queries on-chain reputation scores, and returns them ranked by averageScore (tiebreaker: totalFeedback). Reports failed on-chain queries transparently.
- **Peer/Judge Feedback**: New `POST /v1/reputation/feedback` — authenticated verified agents can submit scored feedback (0-100) for other agents on the Reputation Registry. Self-rating prevented. Anti-spam: 1 feedback per rater per target per 24 hours, plus 10/hour global rate limit.
- **Stats Endpoint Performance**: Replaced `SELECT *` with `COUNT` queries in `/v1/stats` to avoid loading all records into memory.