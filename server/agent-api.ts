...
router.get("/v1/agent-api/briefing", agentApiLimiter, authenticateAgent, async (req: Request, res: Response) => {
  try {
    ...
    lines.push(``);
    lines.push(`  Request service:  POST ${BASE}/v1/agent-requests  { providerPublicKey, description, skillId?, paymentAmount?, paymentToken?, txHash? }`);
    lines.push(`    If you include txHash + paymentAmount, the platform verifies payment onchain automatically.`);
    lines.push(`    paymentToken defaults to SELFCLAW if omitted. Token decimals are fetched onchain.`);
    lines.push(`    Response includes paymentVerified: true/false and verification details.`);
    lines.push(`    txHash must be unique — each transaction can only be used once to prevent double-spend.`);
    lines.push(`  List requests:    GET  ${BASE}/v1/agent-requests`);
    lines.push(`  Get request:      GET  ${BASE}/v1/agent-requests/:id`);
    lines.push(`  Accept request:   PUT  ${BASE}/v1/agent-requests/:id/accept`);
    lines.push(`  Complete request: PUT  ${BASE}/v1/agent-requests/:id/complete`);
    lines.push(`  Cancel request:   PUT  ${BASE}/v1/agent-requests/:id/cancel`);
    lines.push(`  Rate request:     POST ${BASE}/v1/agent-requests/:id/rate { rating (1-5), review }`);
    lines.push(``);
    ...
    lines.push(`           minAmountOut: slippage-adjusted minimum (applied to estimated output)`);
    lines.push(`           fees: { totalFeePct, perLeg: [{ poolId, feePct, feeBps }] }`);
    lines.push(`           poolPrices: { sqrtPriceX96, token0Per1, token1Per0 } per pool`);
    lines.push(`           transactions: unsigned txs (approve → permit2 → swap). Sign each in order.`);
