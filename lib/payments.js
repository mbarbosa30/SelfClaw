...
import { createPaymentMiddleware, getReceivedPayments, getTotalReceived as getMiddlewareReceived } from './x402-middleware.js';
...
export function getPaymentHistory() {
  ...
  const receivedHistory = getReceivedPayments().map(p => ({
    ...p,
    type: 'incoming',
    timestamp: p.receivedAt
  }));
  
  return [...paymentHistory, ...clientHistory, ...receivedHistory].sort((a, b) => 
...
export function getTotalReceived() {
  const middlewareTotal = parseFloat(getMiddlewareReceived() || 0);
  const recordedTotal = paymentHistory
    .filter(p => p.type === 'incoming' && p.status === 'success')
    .reduce((sum, p) => sum + parseFloat(p.amount || 0), 0);
  
  return (middlewareTotal + recordedTotal).toFixed(6);
}
